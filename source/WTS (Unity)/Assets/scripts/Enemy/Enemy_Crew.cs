...
public class Enemy_Crew : MonoBehaviour
{
	float Hp = 100;
	...
	IEnumerator Health ()
	{
		while (true)
		{
			if (suffocating & !(medbay & GameObject.Find("EnemyShip").GetComponent<Enemy_Ship>()._GetMedbayEnergy() > 0))
			{
				Hp -= 5;
			}
			if (medbay)
			{	//f(x)=1.25x^3-5x^2+8.75x
				Hp += 1.25f * (float)Math.Pow(GameObject.Find("EnemyShip").GetComponent<Enemy_Ship>()._GetMedbayEnergy(),3) - 5f * (float)Math.Pow(GameObject.Find("EnemyShip").GetComponent<Enemy_Ship>()._GetMedbayEnergy(),2) + 8.75f * GameObject.Find("EnemyShip").GetComponent<Enemy_Ship>()._GetMedbayEnergy();
			}
			if (Hp <= 0)
				Destroy(gameObject);
			if (Hp > 100)
